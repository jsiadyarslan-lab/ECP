...
class AuthorizationError(ConsoleError):
    """Session authentication failure; HTTP maps this only to 401."""

class ExecutionAuthorizationError(ConsoleError):
    """Execution authorization refusal; HTTP maps this to 403."""
...
class AuthorizedEvaluation:
    ...
    def safe_metadata(self) -> dict[str, Any]:
        return {"evaluation_id": self.evaluation_id, "system_id": self.system_id, "provider": self.provider,
                "adapter": self.adapter, "credential": self.credential.metadata(),
...
class ExecutionStore:
    ...
    def create(self, request_id: str, safe_request: Mapping[str, str]) -> dict[str, Any]:
        with self._lock:
            ...
            now = _utc_now()
            record = {"execution_id": execution_id, "request_id": request_id, "system_id": safe_request["system_id"],
                      "evaluation_id": safe_request["evaluation_id"], "test_id": safe_request["test_id"],
                      "credential_ref": safe_request["credential_ref"], "status": "REQUESTED",
                      "execution_status": "NOT_STARTED", "response_status": "NOT_RECEIVED",
                      "evidence_status": "NOT_GENERATED", "audit_status": "NOT_GENERATED",
                      "persistence_status": "NOT_PERSISTED", "created_at": now, "updated_at": now}
    ...
    def persist(self, execution_id: str, evidence: Mapping[str, Any], audit: Mapping[str, Any]) -> tuple[str, str]:
        ...
        (target / "evidence.json").write_bytes(canonical_bytes(dict(evidence)))
        (target / "audit.json").write_bytes(canonical_bytes(dict(audit)))
...
class LocalGateway:
    def __init__(self, config: GatewayConfig, evaluations: Mapping[str, AuthorizedEvaluation], credential_gateway: CredentialGateway,
                 adapters: Mapping[str, ProviderAdapter] | None = None, store: ExecutionStore | None = None,
        ...
        self._pairing_expires = self._clock() + config.pairing_ttl_seconds
    ...
    def execute(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        try:
            release_request = ScopedReleaseRequest(request_id=safe_request["request_id"], binding_id=evaluation.credential_binding.binding_id,
                target_id=evaluation.credential_binding.target_id, credential_ref=safe_request["credential_ref"], purpose=test.scope,
                scope=frozenset({test.scope}), lease_seconds=60, requested_at=_utc_now())
            lease = self.credential_gateway.release(release_request, evaluation.credential_binding, evaluation.authorization_grant)
            adapter = self.adapters.get(evaluation.adapter) or UnconfiguredAdapter()
            result = dict(adapter.execute(lease, safe_request))
            result.pop("secret", None); result.pop("credential_value", None)
    ...
    def _finalize(self, execution_id: str, evaluation: AuthorizedEvaluation, request: Mapping[str, str], result: Mapping[str, Any], outcome: str, secrets_to_redact: tuple[str, ...] = ()) -> None:
        evidence_id = f"ECP-EVID-CONSOLE-{execution_id.rsplit('-', 1)[-1]}"; audit_id = f"ECP-AUDIT-CONSOLE-{execution_id.rsplit('-', 1)[-1]}"
        ...
        self.store.update(execution_id, status="SUCCESS" if persistence in {"PERSISTED_LOCALLY","PUSH_PENDING"} else "PARTIAL_SUCCESS", evidence_status="GENERATED", audit_status="GENERATED", persistence_status=persistence, persistence_location=location, evidence_id=evidence_id, audit_id=audit_id)
    def make_server(self) -> ThreadingHTTPServer:
        gateway = self
        class Handler(BaseHTTPRequestHandler):
            server_version = "ECP-Local-Gateway/1.0"
            def _origin(self) -> str | None: return self.headers.get("Origin")
            def _check_origin(self) -> bool:
                if self._origin() not in gateway.config.allowed_origins:
                    self._json(403,{"error":"origin denied","state":"SECURITY_BLOCKED"},include_cors=False); return False
                return True
            def _json(self,status:int,payload:Mapping[str,Any],*,include_cors:bool=True)->None:
                ...
                if include_cors and self._origin() in gateway.config.allowed_origins:
                    self.send_header("Access-Control-Allow-Origin",self._origin() or ""); self.send_header("Vary","Origin"); self.send_header("Access-Control-Allow-Headers","Content-Type, X-ECP-Session"); self.send_header("Access-Control-Allow-Methods","GET, POST, OPTIONS")
            ...
            def log_message(self,fmt:str,*args:Any)->None: print("ECP gateway: "+fmt%tuple(args))
        self._httpd=ThreadingHTTPServer(("127.0.0.1",self.config.port),Handler); return self._httpd
    def serve_forever(self,server:ThreadingHTTPServer|None=None)->None: (server or self.make_server()).serve_forever()
...
def _validate_request(request:Mapping[str,Any])->None:
    ...
    if any(not isinstance(request[key],str) or not request[key].strip() for key in _ALLOWED_REQUEST_KEYS): raise ConsoleError("request identifiers must be non-empty strings")
...
def _safe_exception_message(error:BaseException,lease:SecretLease|None=None)->str:
    ...
    return SecretRedactionFilter((lease.value,)).redact(message) if lease is not None else message
...
def default_gateway_config()->GatewayConfig:
    ...
    port=int(os.environ.get("ECP_GATEWAY_PORT",str(DEFAULT_PORT))); root=os.environ.get("ECP_ARTIFACT_ROOT")
...
def build_demo_gateway(config:GatewayConfig|None=None)->LocalGateway:
    """Compose the demo from the existing private synthetic credential store.

    The catalog identity and CredentialGateway share this fixture. The value is
    generated only in process and is never placed in the repository or browser.
    """
    ...
    gateway.provision_for_testing(identity.credential_id, secrets.token_urlsafe(32))
...
def main(argv:list[str]|None=None)->int:
    parser=argparse.ArgumentParser(description="Run the loopback-only ECP local evaluation gateway"); parser.add_argument("--port",type=int,default=None); parser.add_argument("--artifact-root",default=None); args=parser.parse_args(argv)
    ...
    gateway=build_demo_gateway(config); server=gateway.make_server()
    print(f"ECP Local Gateway listening on 127.0.0.1:{config.port}"); print(f"PAIRING CODE (expires in {config.pairing_ttl_seconds}s): {gateway.pairing_code}"); print("Allowed origins: "+", ".join(sorted(config.allowed_origins))); gateway.serve_forever(server); return 0
