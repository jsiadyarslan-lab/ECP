"""Explicit version-compatibility tests (0.2.0 → 0.3.0 additive bundle).

0.1.x and 0.2.x artifacts are NOT silently reinterpreted and NOT
...
including the four M3-CA0 review-layer contracts introduced at 0.3.0.
"""
...
def test_matrix_covers_all_known_object_types():
    assert set(SCHEMA_VERSIONS) == (
        set(V010_CONTRACTS) | set(V020_CONTRACTS) | set(V030_CONTRACTS)
    )
    assert len(SCHEMA_VERSIONS) == 16


def test_v010_contracts_accept_all_bundle_versions():
    for name in V010_CONTRACTS:
        assert allowed_schema_versions(name) == ("0.1.0", "0.2.0", "0.3.0"), name


def test_v020_contracts_accept_020_and_030():
    for name in V020_CONTRACTS:
        assert allowed_schema_versions(name) == ("0.2.0", "0.3.0"), name


def test_v030_contracts_accept_only_030():
    for name in V030_CONTRACTS:
        assert allowed_schema_versions(name) == ("0.3.0",), name
...
def test_protocol_axis_accepts_010_020_030():
    assert PROTOCOL_VERSIONS == ("0.1.0", "0.2.0", "0.3.0")
...
def test_version_issues_rejects_future_protocol_version():
    document = {"ecp_object": "case", "protocol_version": "0.4.0"}
    issues = version_issues(document)
